            ...
            )
        {
            ...
            while(c.MAX_DEPTH < MaxDepth) {
                ...
                for(n->setAB(lower, upper); 
                     ...
                     * haven't met, and alpha
                     * < beta, do the another
                     * rollout at the same depth.
                     */
                    if(!n->converged() && 
                       !n->updateAB())
                        continue;
                     ...
                     * If V- and V+ have met,
                     * or alpha >= beta, see
                     * if our score fell outside
                     * the aspiration window.
                     * If so, relax the window.
                     * If not, trust the score 
                     * and break the aspiration
                     * loop.
                     */
                    if(n->getScore() <= lower) {
                        lower = std::max
